...
describe("FileUploadZone", () => {
  ...
  beforeEach(() => {
    ...
    mockGenerateUploadUrl.mockResolvedValue({ uploadUrl: "https://upload.test" })
  ...
  it("rejects unsupported file extensions", async () => {
    ...
    const input = screen.getByTestId("attachment-file-input")
    const file = new File(["bin"], "payload.exe", { type: "application/octet-stream" })
    await user.upload(input, file)
  ...
  it("updates queued jobs to ready and removes them after 5 seconds", async () => {
    ...
    listAttachmentsValue = [{ _id: "att1", status: "ready", optimizationError: null }]
  ...
  it("removes queued jobs after 30 seconds when still not ready", async () => {
    ...
    const input = screen.getByTestId("attachment-file-input")
    const file = new File(["pdf"], "still-processing.pdf", { type: "application/pdf" })
    await user.upload(input, file)

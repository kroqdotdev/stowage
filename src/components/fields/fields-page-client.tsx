...
export function FieldsPageClient() {
  ...
  const createFieldDefinition = useMutation(api.customFields.createFieldDefinition)
  const updateFieldDefinition = useMutation(api.customFields.updateFieldDefinition)
  const deleteFieldDefinition = useMutation(api.customFields.deleteFieldDefinition)
  const reorderFieldDefinitions = useMutation(api.customFields.reorderFieldDefinitions)
    ...
    try {
      ...
      toast.error(
        mapFieldDefinitionError(
          error,
          editorMode === "create" ? "Unable to create field" : "Unable to update field",
        ),
      )
    } finally {
      setSubmitting(false)
  ...
  async function saveOrder(nextIds: string[]) {
    ...
    try {
      ...
      toast.error(
        mapFieldDefinitionError(error, "Unable to save field order"),
      )
    } finally {
      setReordering(false)
      setDraggingId(null)
      ...
      <section className="rounded-xl border border-border/70 bg-background p-5 shadow-sm">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="space-y-1">
            <h2 className="text-lg font-semibold tracking-tight">Field definitions</h2>
            <p className="text-sm text-muted-foreground">
              Define reusable custom fields for assets.
            </p>
            {!canManage ? (
              <p className="text-xs text-muted-foreground">Only admins can manage fields.</p>
            ) : null}
          </div>
          {canManage ? (
            <Button
                  ...
                  >
                    <td className="px-3 py-2">
                      <div className="inline-flex items-center gap-1 rounded-md border border-border/70 bg-muted/20 px-1.5 py-1 text-xs text-muted-foreground">
                        ...
                        <span className="font-mono">{definition.sortOrder + 1}</span>
                      </div>
                    </td>
                      ...
                      {canManage ? (
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => openEdit(definition._id as string)}>

...
import { getTagIdsForAsset, listTagsForAsset, replaceAssetTags } from "./assetTags"
import {
  ASSET_STATUSES,
...
const optionalCategoryIdValidator = v.optional(v.union(v.id("categories"), v.null()))
const optionalLocationIdValidator = v.optional(v.union(v.id("locations"), v.null()))
...
const optionalSortDirectionValidator = v.optional(v.union(v.literal("asc"), v.literal("desc")))
...
function pickSort(
  ...
  return {
    ...
    compare(
      ...
      if (by === "assetTag") {
        return a.assetTag.localeCompare(b.assetTag, undefined, { sensitivity: "base" }) * multiplier
      }
      ...
      return a.status.localeCompare(b.status, undefined, { sensitivity: "base" }) * multiplier
    },
  }
}
...
async function requireAsset(ctx: QueryCtx | MutationCtx, assetId: Id<"assets">) {
...
async function getFieldDefinitions(ctx: QueryCtx | MutationCtx) {
  const definitions = (await ctx.db.query("customFieldDefinitions").collect()) as FieldDefinitionRow[]
  return definitions
}
...
function normalizeStringCustomFieldValue(value: AssetCustomFieldValue) {
  ...
  if (typeof value !== "string") {
    throwAssetError("INVALID_CUSTOM_FIELD_VALUE", "Custom field value must be text")
  }
...
function normalizeNumberCustomFieldValue(value: AssetCustomFieldValue) {
  ...
  if (typeof value !== "number" || !Number.isFinite(value)) {
    throwAssetError("INVALID_CUSTOM_FIELD_VALUE", "Custom field value must be numeric")
  }
...
function normalizeCheckboxCustomFieldValue(value: AssetCustomFieldValue) {
  ...
  if (typeof value !== "boolean") {
    throwAssetError("INVALID_CUSTOM_FIELD_VALUE", "Custom field value must be true or false")
  }
...
function normalizeCustomValueByType(
  ...
  if (fieldDefinition.fieldType === "number" || fieldDefinition.fieldType === "currency") {
  ...
  if (fieldDefinition.fieldType === "url") {
    try {
      ...
      throwAssetError("INVALID_CUSTOM_FIELD_VALUE", `${fieldDefinition.name} must be a valid URL`)
    }
  }
...
async function validateAndNormalizeCustomFieldValues(
  ...
  for (const [fieldId, rawValue] of Object.entries(normalizedInput)) {
    ...
    if (!definition) {
      throwAssetError("INVALID_CUSTOM_FIELD", "Custom field definition not found")
    }
  ...
  for (const definition of definitions) {
    ...
    if (definition.fieldType === "checkbox") {
      if (value === null || value === undefined) {
        throwAssetError("REQUIRED_CUSTOM_FIELD", `${definition.name} is required`)
      }
      continue
    }
...
async function updateFieldUsageCounts(
  ...
  for (const fieldId of previousUsed) {
    ...
    updates.push(
      (async () => {
        const definition = (await ctx.db.get(fieldDefinitionId)) as FieldDefinitionRow | null
        if (!definition) {
          return
        }
...
async function queryAssetsByPrimaryFilter(
  ...
  if (args.categoryId !== undefined) {
    return (await ctx.db
      .query("assets")
      .withIndex("by_categoryId", (q) => q.eq("categoryId", args.categoryId ?? null))
      .collect()) as AssetRow[]
  }
  ...
  if (args.locationId !== undefined) {
    return (await ctx.db
      .query("assets")
      .withIndex("by_locationId", (q) => q.eq("locationId", args.locationId ?? null))
      .collect()) as AssetRow[]
  }
  ...
  if (tagAssetIds && tagAssetIds.size > 0 && tagAssetIds.size <= TAG_ONLY_DIRECT_FETCH_LIMIT) {
    const rows = await Promise.all(Array.from(tagAssetIds).map((assetId) => ctx.db.get(assetId)))
...
async function listAssetRowsByFilters(ctx: QueryCtx, args: AssetListFilterArgs) {
  const normalizedTagIds = normalizeTagFilter(args.tagIds)
  const tagAssetIds = await getIntersectedAssetIdsForTags(ctx, normalizedTagIds)
  ...
  return rows.filter((row) => {
    if (args.categoryId !== undefined && (args.categoryId ?? null) !== row.categoryId) {
      return false
    }
    ...
    if (args.locationId !== undefined && (args.locationId ?? null) !== row.locationId) {
      return false
    }

    if (normalizedTagIds.length > 0 && tagAssetIds && !tagAssetIds.has(row._id)) {
      return false
    }
...
async function buildAssetListViews(ctx: QueryCtx, assetRows: AssetRow[]) {
  ...
  const categoryIds = Array.from(
    new Set(
      assetRows
        ...
        .filter((categoryId): categoryId is Id<"categories"> => categoryId !== null),
    ),
  )
  ...
  return assetRows.map((asset) => {
    const category = asset.categoryId ? categoryById.get(asset.categoryId) ?? null : null
    const location = asset.locationId ? locationById.get(asset.locationId) ?? null : null
...
function matchesAssetSearch(
  ...
  return [row.name, row.assetTag, row.notes ?? "", ...row.tagNames].some((value) =>
...
export const createAsset = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const customFieldValues = await validateAndNormalizeCustomFieldValues(ctx, args.customFieldValues)
    ...
    return {
      ...
      location: location
        ? {
            ...
            parentId: ((location as LocationRow).parentId ?? null) as Id<"locations"> | null,
...
export const updateAsset = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const name = args.name === undefined ? asset.name : requireAssetName(args.name)
    const normalizedName = normalizeAssetNameKey(name)
    const categoryId = args.categoryId === undefined ? asset.categoryId : args.categoryId ?? null
    const locationId = args.locationId === undefined ? asset.locationId : args.locationId ?? null
    ...
    const notes = args.notes === undefined ? asset.notes : normalizeAssetNotes(args.notes)
    const customFieldValues =
      args.customFieldValues === undefined
        ? asset.customFieldValues
        : await validateAndNormalizeCustomFieldValues(ctx, args.customFieldValues)
    ...
    await updateFieldUsageCounts(ctx, asset.customFieldValues, customFieldValues)
...
export const deleteAsset = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    await Promise.all(links.map((link) => ctx.db.delete(link._id as Id<"assetTags">)))

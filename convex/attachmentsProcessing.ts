  ...
  handler: async (ctx, args) => {
    ...
    try {
      ...
      if (blob.size > MAX_ATTACHMENT_UPLOAD_BYTES) {
        throw new Error("This file is too large. Upload files up to 25 MB.")
      }

      const inputBytes = new Uint8Array(await blob.arrayBuffer())
      let optimized: OptimizedBinary = {
        bytes: inputBytes,
      ...
      const shouldReplaceStoredFile =
        optimized.bytes.byteLength !== inputBytes.byteLength ||
        optimized.fileType !== attachment.fileType ||
        optimized.fileExtension !== attachment.fileExtension ||
        optimized.fileName !== attachment.fileName

      const newStorageId = shouldReplaceStoredFile
        ? await ctx.storage.store(
            new Blob([Buffer.from(optimized.bytes)], { type: optimized.fileType }),
          )
        : null
      ...
      await ctx.runMutation(internal.attachments.markAttachmentReady, {
        ...
        fileSizeOptimized: optimized.bytes.byteLength,
      })
    } catch (error) {
      const failure = await ctx.runMutation(internal.attachments.markAttachmentFailed, {
        attachmentId: args.attachmentId,

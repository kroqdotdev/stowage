...
describe("auth route logic", () => {
  ...
  it("redirects auth pages appropriately", () => {
    expect(
      getLoginPageRedirect({ firstRun: true, isAuthenticated: false }),
    ).toBe("/setup");
    expect(
      getLoginPageRedirect({ firstRun: false, isAuthenticated: true }),
    ).toBe("/dashboard");
    expect(
      getLoginPageRedirect({ firstRun: false, isAuthenticated: false }),
    ).toBeNull();

    expect(
      getSetupPageRedirect({ firstRun: false, isAuthenticated: false }),
    ).toBe("/login");
    expect(
      getSetupPageRedirect({ firstRun: false, isAuthenticated: true }),
    ).toBe("/dashboard");
    expect(
      getSetupPageRedirect({ firstRun: true, isAuthenticated: false }),
    ).toBeNull();
  });
});

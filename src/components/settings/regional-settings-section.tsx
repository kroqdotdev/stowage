...
export function RegionalSettingsSection() {
  ...
  const [selectedFormat, setSelectedFormat] = useState<AppDateFormat | null>(null)
  const [saving, setSaving] = useState(false)

  const persistedFormat =
    (settings?.dateFormat as AppDateFormat | undefined) ?? DEFAULT_APP_DATE_FORMAT
  ...
  return (
    <section className="rounded-xl border border-border/70 bg-background p-5 shadow-sm">
      <div className="space-y-1">
        <h2 className="text-lg font-semibold tracking-tight">Regional preferences</h2>
      ...
      {settings === undefined ? (
        <div className="mt-4 text-sm text-muted-foreground">Loading preferences...</div>
      ) : (
        <div className="mt-4 space-y-4">
          <div className="space-y-1.5">
            ...
            <select
              ...
              onChange={(event) => setSelectedFormat(event.target.value as AppDateFormat)}
              disabled={saving}
            >
          ...
          <div className="rounded-md border border-border/70 bg-muted/20 px-3 py-2 text-sm text-muted-foreground">
            <div>
              ...
              <span className="font-medium text-foreground">
                {formatDateFromTimestamp(Date.UTC(2026, 1, 27), effectiveFormat)}
              </span>
            </div>
            {settings.updatedAt ? (

...
test.describe.serial("phase 5 assets", () => {
  test("protected asset routes redirect when unauthenticated", async ({ page }) => {
  ...
  test("can create, edit, and update status for an asset", async ({ page }) => {
    ...
    test.skip(!email || !password, "Set E2E_AUTH_EMAIL and E2E_AUTH_PASSWORD to run the assets e2e test")
    ...
    await page.locator("tbody tr").filter({ hasText: updatedName }).first().click()
    await page.waitForURL(/\/assets\/.+$/, { timeout: 15_000 })
    await expect(page.getByRole("heading", { name: updatedName })).toBeVisible()
    await expect(page.getByText("Retired")).toBeVisible()
    ...
    await page.waitForURL((url) => getLeafPathSegment(url.toString()) === "assets", {
      timeout: 20_000,
    })

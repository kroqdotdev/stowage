...
async function requireServiceRecord(
  ...
  const record = (await ctx.db.get(
    serviceRecordId,
  )) as ServiceRecordAccessRow | null;
  if (!record) {
...
function ensureServiceRecordMutationAccess(
  actor: Awaited<ReturnType<typeof requireAuthenticatedUser>>,
  record: ServiceRecordAccessRow,
) {
  if (actor.role === "admin") {
    return;
  }
...
export const createAttachment = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const record = await requireServiceRecord(ctx, args.serviceRecordId);
    ensureServiceRecordMutationAccess(actor, record);
...
export const deleteAttachment = mutation({
  ...
  handler: async (ctx, args) => {
    const actor = await requireAuthenticatedUser(ctx);
    const attachment = await requireServiceRecordAttachment(
      ctx,
      args.attachmentId,
    );

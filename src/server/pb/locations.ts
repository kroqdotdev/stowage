import { normalizeCatalogName, normalizeOptionalText } from "./catalog";
...
export function replaceLocationPathPrefix(
  ...
  if (existingPath === oldPrefix) return newPrefix;
  const childPrefix = `${oldPrefix}${LOCATION_PATH_SEPARATOR}`;
  if (!existingPath.startsWith(childPrefix)) return existingPath;

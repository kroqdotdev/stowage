...
export function AssetEditPageClient({ assetId }: { assetId: Id<"assets"> }) {
  ...
  const deleteSchedule = useMutation(api.serviceSchedules.deleteSchedule);
  ...
  const existingSchedule = useQuery(api.serviceSchedules.getScheduleByAssetId, {
    assetId,
  });
  ...
  useEffect(() => {
    ...
    setScheduleSeeded(true);
  }, [existingSchedule, scheduleSeeded]);

...
import { AssetFilters, type AssetFiltersState } from "@/components/assets/asset-filters"
import {
  AssetTable,
  type AssetSortBy,
  type AssetSortDirection,
} from "@/components/assets/asset-table"
import type { AssetFilterOptions, AssetListItem } from "@/components/assets/types"
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-border/60 bg-muted/15 px-3 py-2 text-xs text-muted-foreground">
        {selectedCount === 0 ? (
          ...
          <span>
            {selectedCount} selected: {selectedAssetNames.slice(0, 3).join(", ")}
            {selectedAssetNames.length > 3 ? ` +${selectedAssetNames.length - 3} more` : ""}
          </span>
        )}
      </div>
    </div>

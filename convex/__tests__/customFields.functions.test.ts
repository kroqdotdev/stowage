...
describe("customFields functions", () => {
  ...
  it("creates and lists definitions in sort order", async () => {
    ...
    expect(rows.map((row) => row.name)).toEqual(["Serial number", "Purchase date"])
  ...
  it("rejects reorder payloads with duplicate IDs", async () => {
    ...
    const second = await admin.mutation(api.customFields.createFieldDefinition, {
      name: "Second",
      fieldType: "text",
      options: [],
      required: false,
  ...
  it("blocks unsafe type changes and deletes when field is in use", async () => {
    const admin = asUser(t, adminId)
    const created = await admin.mutation(api.customFields.createFieldDefinition, {
      name: "Condition",
      fieldType: "dropdown",
      options: ["New", "Used"],
      required: false,

...
async function getLocationOrThrow(ctx: MutationCtx, locationId: Id<"locations">) {
...
async function assertUniqueSiblingName(
  ...
  if (duplicate) {
    throw new ConvexError("A location with this name already exists at this level")
  }
}
...
async function patchDescendantPaths(
  ...
  const allLocations = (await ctx.db.query("locations").collect()) as LocationRow[]
  const descendants = allLocations.filter(
    (location) => location.path !== oldPath && location.path.startsWith(`${oldPath} / `),
  )
...
async function updateLocationCore(
  ...
  const nextName = args.name === undefined ? existing.name : requireLocationName(args.name)
  const nextNormalizedName = normalizeLocationNameKey(nextName)
  const nextDescription =
    args.description === undefined
      ? existing.description
      : normalizeLocationDescription(args.description)
  const nextParentId = args.parentId === undefined ? existing.parentId : args.parentId
  ...
  const byId = new Map(
    allLocations.map((location) => [
      ...
      { _id: location._id as string, parentId: location.parentId as string | null },
    ]),
  )
  ...
  if (
    ...
    throw new ConvexError("A location cannot be moved inside one of its descendants")
  }

  const parent = await getParentLocation(ctx, nextParentId ?? null)
  await assertUniqueSiblingName(ctx, parent?._id ?? null, nextNormalizedName, existing._id)
...
export const listLocations = query({
  ...
  handler: async (ctx) => {
    ...
    const locations = (await ctx.db.query("locations").collect()) as LocationRow[]
    return locations
      .slice()

...
test.describe.serial("phase 6 attachments", () => {
  test("uploads, lists, downloads, and deletes attachments", async ({ page }) => {
    const email = process.env.E2E_AUTH_EMAIL
    const password = process.env.E2E_AUTH_PASSWORD

    test.skip(!email || !password, "Set E2E_AUTH_EMAIL and E2E_AUTH_PASSWORD to run the attachments e2e test")

    const landing = await getLanding(page)
    test.skip(landing !== "login", "Attachments e2e requires setup to be complete")
    ...
    const pdfCard = page.locator("article").filter({ hasText: "manual.pdf" }).first()
    const downloadHref = await pdfCard.getByRole("link", { name: "Download" }).getAttribute("href")
    ...
    await expect(page.locator("article").filter({ hasText: "manual.pdf" })).toHaveCount(0, {
      timeout: 15_000,
    })
  })
})
